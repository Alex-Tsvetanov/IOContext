...
// Enable `co_yield std::monostate{};` to reschedule on the pool
std::suspend_always Task::promise_type::yield_value(std::monostate) noexcept {
  auto h = Task::handle_t::from_promise(*this);
  // Re-enqueue ourselves to the same pool
  if (auto tp = pool.lock()) {
    // enqueue_ is private; friend access via ThreadPool declaration
    tp->enqueue_(h);
  }
  // Suspend now; worker will pick something else
  return {};
}

//--------------------------------------------------------------
// Example usage (now NO pool arg; use co_yield std::monostate{})
//--------------------------------------------------------------
#include <iostream>
#include <chrono>
...
Task stepper(int id, int steps, int work_ms) {
  for (int i = 0; i < steps; ++i) {
    ...
    co_yield {}; // reschedule on the same pool
  }
}
