...
static ssize_t send_callback([[maybe_unused]] nghttp2_session* session, const uint8_t* data, size_t length,
...
static int alpn_select_cb([[maybe_unused]] SSL* ssl, const unsigned char** out, unsigned char* outlen,
...
void init_ssl()
{
  ...
  SSL_CTX_use_certificate_file(ssl_ctx, "/mnt/c/users/alext/Documents/GitHub/IOContext/tests/unix/http2/cert.pem",
                               SSL_FILETYPE_PEM);
  SSL_CTX_use_PrivateKey_file(ssl_ctx, "/mnt/c/users/alext/Documents/GitHub/IOContext/tests/unix/http2/key.pem",
                              SSL_FILETYPE_PEM);
...
struct Worker
{
  ...
  void run()
  {
    ...
    while (true)
    {
      ...
      // Cleanup closed
      conns.erase(std::remove_if(conns.begin(), conns.end(), [](auto& c) { return c->closed; }), conns.end());
    }
  }
};
